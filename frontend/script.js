...
// Text ve çıktı alanları
const userText = document.getElementById("userText");
const correctedOutput = document.querySelector(".corrected");
//const levelBadge = document.getElementById("levelBadge");
...
// Sahte backend fonksiyonu (örnek düzeltme)
function mockGrammarCorrection(text) {
  let mistakes = 0;
  let corrected = text;

  // basit bir örnek düzeltme mantığı
  if (text.toLowerCase().includes("he go")) {
    corrected = text.replace(/he go/gi, "He goes");
    mistakes++;
  }
...
// Kullanıcı metnini düzelt
function handleCorrection() {
  const text = userText.value.trim();
  if (!text) return;

  const result = mockGrammarCorrection(text);
  correctedOutput.innerHTML = result.corrected;

  // Hataları istatistiklere ekle
  updateStats(result.mistakes);

  // Seviye belirle
  const level = result.mistakes === 0 ? "Advanced" :
                 result.mistakes === 1 ? "Intermediate" : "Beginner";
...
function updateStats(mistakes) {
  totalSentences++;
  totalMistakes += mistakes;

  const accuracy = totalSentences > 0
    ? Math.max(0, Math.round(((totalSentences - totalMistakes) / totalSentences) * 100))
    : 0;
...
function addToHistory(original, corrected) {
  ...
  item.innerHTML = `
    <div class="texts">
      <p class="history-original">${original}</p>
      <p class="history-corrected">${corrected}</p>
    </div>
    <div class="meta">#${totalSentences}</div>
  `;
...
// 🎙 Başlat butonu (örnek simülasyon)
startBtn.addEventListener("click", () => {
  ...
  setTimeout(() => {
    ...
    // Simülasyon: kullanıcı cümle söyledi varsayalım
    userText.value = "He go to school every day";
    handleCorrection();
  }, 2000);
});

// Kaydet butonu (geçmişe ekleme)
saveBtn.addEventListener("click", () => {
  handleCorrection();
});

// Dinle butonu (metni seslendir)
